...
void BigPixelCanvas::DrawCircle(const wxPoint& center, int radius, wxDC& dc)
{
    ...
    // Loop prossegue ate o fim do segundo octante
    // ultimo ponto eh x == y
    while (y >= x) {
        ++x;
        
        if (d > 0) {
        // Ponto medio esta fora da circunferencia
        // escolher ponto sudeste
            --y;
            d += 2*(x - y) + 5;
        }
        else {
        // Ponto medio esta dentro da circunferencia
        // escolher ponto leste 
            d += 2*x + 3;   
        }
